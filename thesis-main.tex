...
Il Capitolo 3 descrive il processo di face morphing nello spazio degli embedding, analizzando le tecniche di interpolazione lineare e sferica, e mostra l’adattamento delle immagini generate agli standard ICAO mediante una fase di preprocessing finalizzata alla rimozione e normalizzazione dello sfondo.
...
Negli ultimi anni sono stati sviluppati numerosi approcci al face recognition basati su tecniche di deep learning, che differiscono principalmente per l’architettura della rete neurale e per la funzione di perdita utilizzata durante l’addestramento.
Tra i modelli più noti vi sono DeepFace, VGG-Face, FaceNet, OpenFace, DeepID, ArcFace e CosFace.

Tali modelli condividono una struttura generale basata su \textit{Deep Convolutional Neural Network} (DCNN).
Attraverso l'uso di reti convoluzionali il sistema è in grado di tradurre i pixel di un immagine in un vettore numerico compatto, detto \textit{embedding}. Questa rappresentazione contiene informazioni più astratte legate all'identità della persona.
...
Il modello di riconoscimento facciale ArcFace (\textit{Additive Angular Margin Loss}) affina il processo di separazione proiettando gli embedding su una ipersfera unitaria. Questo significa che i vettori non sono più distribuiti liberamente in $\mathbb{R^d}$, ma sono vincolati a stare sulla superficie della sfera $S^{d-1}$. In questo spazio la somiglianza tra due vettori non dipende dalla lunghezza, ma solo dall'angolo tra di essi.
...
\begin{itemize}
    ...
    Geometricamente, la LERP non segue la curva della sfera, ma si muove lungo la corda che collega i due punti. Questo introduce un errore geometrico, perché l'interpolazione non rispetta la struttura dello spazio in cui il sistema misura la somiglianza tra identità \cite{10007987}.
...
\begin{figure}[H]
    ...
    \caption{Confronto geometrico tra Interpolazione Lineare e Interpolazione Sferica}
...
La conformità delle immagini viene definita attraverso vincoli descritti nello standard ISO e nelle linee guida ICAO, formalizzati in una serie di test che verificano aspetti geometrici e fotografici dell'immagine.
...
Per ottenere immagini conformi ai requisiti relativi allo sfondo previsti dallo standard ISO/ICAO, è stato necessario introdurre una fase di pre-processing finalizzata alla normalizzazione del background. A tal fine, sono stati adottati modelli di \textit{background removal}, ovvero sistemi basati su tecniche di segmentazione automatica in grado di separare il soggetto dallo sfondo e generare una maschera utile alla sua sostituzione con uno sfondo uniforme.
...
\begin{itemize}
    \item \textbf{Binary / single-class segmentation:} U\textsuperscript{2}-Net, SAM2.
    ...
    Nel progetto sono stati testati modelli come \textbf{U\textsuperscript{2}-Net}, utilizzato nella variante addestrata per la segmentazione di soggetti umani, noto per la buona capacità di estrarre dettagli fini grazie a una struttura encoder-decoder annidata, e \textbf{SAM2}, basato su un approccio di segmentazione generalista \textit{prompt-based}, in cui la predizione viene guidata tramite input come punti o bounding box, permettendo di ottenere maschere accurate anche su categorie non viste durante l'addestramento.

    \item \textbf{Multi-class segmentation:} SegFormer, BiSeNet.
    ...
    Tra i modelli testati rientrano \textbf{SegFormer}, basato su un backbone Transformer e reso specializzato sul volto tramite addestramento su un dataset facciale, e \textbf{BiSeNet}, con backbone ResNet, progettato per combinare informazioni di dettaglio spaziale e contesto globale, utilizzando ResNet come estrattore di caratteristiche per produrre una mappa di segmentazione multi-classe a livello di pixel.

\end{itemize}

\subsubsection{Modelli di alpha matting:  \normalfont MODNet, BEN2}
...
In questo contesto sono stati valutati modelli come \textbf{MODNet}, progettato specificamente per il portrait matting in tempo reale, e \textbf{BEN2}, utilizzato per ottenere una separazione più precisa nelle zone di transizione tra soggetto e sfondo
...
Questo significa che la maschera generata deve isolare correttamente il soggetto, preservandone la struttura complessiva, i contorni e i dettagli significativi,
evitando sia l’inclusione di porzioni di sfondo (\textit{false positive}), che comprometterebbe la pulizia del risultato, sia l’eliminazione di parti del volto (\textit{false negative}), che potrebbe comportare una perdita di informazioni rilevanti.
...
In altre parole, la rimozione dello sfondo non deve ridurre né aumentare artificialmente la probabilità che un’immagine venga accettata da un sistema biometrico.
...
Il \textit{FEI Face Database} è un dataset di immagini facciali sviluppato presso l'Artificial Intelligence Laboratory della FEI University, comprendente fotografie relative a 200 individui distinti maschili e femminili. Tutte le immagini sono state acquisite in ambiente controllato: sfondo bianco uniforme, illuminazione relativamente costante, risoluzione standard. Per ogni soggetto sono disponibili 14 immagini che lo ritraggono con diverse pose del capo e differenti espressioni facciali.
...
\hline
\includegraphics[width=0.18\textwidth]{figures/ides/5-11.jpg} &
\includegraphics[width=0.18\textwidth]{figures/caso_ideale/M_5-11_23-11.jpg} &
\includegraphics[width=0.18\textwidth]{figures/ides/23-11.jpg} &
\includegraphics[width=0.18\textwidth]{figures/caso_ideale/D_5-11.jpg} &
\includegraphics[width=0.18\textwidth]{figures/caso_ideale/D_23-11.jpg} \\
\hline
\end{tabular}
...
\hline
\includegraphics[width=0.18\textwidth]{figures/caso_ideale/M_5-11_23-11.jpg} &
\includegraphics[width=0.18\textwidth]{figures/ides/5-12.jpg} &
\includegraphics[width=0.18\textwidth]{figures/caso_reale/D_M_5-12.jpg} &
\includegraphics[width=0.18\textwidth]{figures/ides/23-11.jpg} \\
\hline
\end{tabular}
...
\hline
\includegraphics[width=0.18\textwidth]{figures/caso_ideale/M_5-11_23-11.jpg} &
\includegraphics[width=0.18\textwidth]{figures/ides/23-12.jpg} &
\includegraphics[width=0.18\textwidth]{figures/caso_reale/D_M_23-12.jpg} &
\includegraphics[width=0.18\textwidth]{figures/ides/5-11.jpg} \\
\hline
\end{tabular}
...
\begin{itemize}
    \item DOC rappresenta il documento appartenente realmente al soggetto ID1
    \item LIVE è l'immagine dello stesso soggetto ID1 con espressione differente rispetto al documento
\end{itemize}
